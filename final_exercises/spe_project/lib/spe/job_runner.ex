defmodule SPE.JobRunner do
  ...
  defp schedule_ready_tasks(state) do
    ...
    Enum.each(ready_tasks, fn task ->
      dependencies = dependency_results(task.name, state)
      SPE.WorkerPool.run(self(), state.job_id, task, dependencies, state.pubsub_name)
    end)
  ...
  defp dependencies(tasks) do
    initial = Map.new(tasks, &{&1.name, []})

    Enum.reduce(tasks, initial, fn task, acc ->
  ...
  defp dependents(tasks) do
    ...
    Enum.reduce(tasks, initial, fn task, acc ->
      Map.put(acc, task.name, task.enables)
    end)
  end
